            ...
            try:
                ...
                for i, source in enumerate(sources[:2]):
                    if isinstance(source, dict):
                        print(
                            f"    Source {i+1}: {source.get('text', 'No text')} - {source.get('link', 'No link')}"
                        )
                    else:
    ...
    def test_06_course_specific_queries(self):
        ...
        for query in course_queries:
            ...
            try:
                ...
                course_mentioned = any(
                    course in response for course in available_courses
                )
                if course_mentioned:
    ...
    def test_07_comparison_queries(self):
        ...
        for query in comparison_queries:
            ...
            try:
                ...
                print(f"  Response length: {len(response)}")
                print(
                    f"  Sources from multiple courses: {len(set(source.get('text', '').split(' - ')[0] for source in sources if isinstance(source, dict)))}"
                )
    ...
    def test_08_outline_tool_functionality(self):
        ...
        for query in outline_queries:
            ...
            try:
                ...
                has_lesson_numbers = any(
                    f"lesson {i}" in response.lower() or f"{i}." in response
                    for i in range(1, 10)
                )
    ...
    def test_10_realistic_user_scenarios(self):
        ...
        try:
            ...
            response2, _ = self.rag_system.query(
                "Which course should I start with for AI applications?"
            )
        ...
        try:
            ...
            response1, _ = self.rag_system.query(
                "I want to learn about MCP", session_id
            )
            response2, _ = self.rag_system.query("What's in lesson 1?", session_id)
            response3, _ = self.rag_system.query(
                "Can you explain that concept more?", session_id
            )
        ...
        try:
            response1, _ = self.rag_system.query(
                "How does vector search compare to traditional search?"
            )
            response2, _ = self.rag_system.query(
                "What are the pros and cons of different embedding approaches?"
            )
