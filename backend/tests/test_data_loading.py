...
class TestDataLoadingAndState(unittest.TestCase):
    ...
    def test_01_docs_folder_exists_and_has_content(self):
        ...
        self.assertTrue(
            os.path.exists(docs_full_path), f"Docs folder not found at {docs_full_path}"
        )
        ...
        self.assertGreater(
            len(course_files), 0, f"No course files found in {docs_full_path}"
        )
    ...
    def test_05_vector_store_search_functionality(self):
        ...
        try:
            ...
            # Test simple search
            test_queries = ["machine learning", "introduction", "lesson 1", "what is"]
        ...
        try:
            ...
            for query in test_queries:
                ...
                try:
                    ...
                    print(
                        f"Answer length: {len(answer) if isinstance(answer, str) else 'N/A'}"
                    )
                    ...
                    if isinstance(answer, str):
                        if "query failed" in answer.lower():
                            ...
                            preview = (
                                answer[:150] + "..." if len(answer) > 150 else answer
                            )
                            print(f"Preview: {preview}")
